 ...
 * 二、对象在内存中的状态
 *      1.可到达状态：
 *      2.可恢复状态：如果程序中某个对象不再有任何引用变量引用它，它就进入了可恢复状态。在这种状态下，垃圾回收机制准备收回改对象的内存
 *      3.不可达状态：当对象与所有引用变脸的关联都被切断，且系统已经调用所有对象的finalize()方法后依然没有使对象变成可到达状态，那么
 *                  这个对象将永久性的失去引用，最后变成不可达状态。只有对象处于不可达状态的时候，系统才会真正的回收该对象的资源。
 *
 ...
 *    系统完全可能并不立刻执行，但是也不会对程序的建议完全置之不理，垃圾回收机制会在收到通知后，尽快进行垃圾回收。
 *    强制系统垃圾收回有两种方法:
 *      1.System.gc();
